...
$sql = "SELECT title_art, description_art, text_art, code_art, date_create_art FROM articulos LIMIT 20";
...
<div class="container">
    ...
    <main class="my-2">
        ...
        <div class="w-100 d-md-flex justify-content-md-between font-weight-bold jumbotron my-3">
            <p>Buscar blog por su nomre</p>
            <form class="form-inline" method="get">
                <input class="form-control mr-sm-2" name="title" type="search" placeholder="Search" aria-label="Search" required>
                <button type="submit" name="btn-search" class="btn btn-outline-success my-2 my-sm-0" >Search</button>
        ...
        <?php
            ...
            if (isset($_GET['btn-search'])){
                $connection2 = mysqli_connect('localhost', 'root', '', 'prueba');
                mysqli_set_charset($connection2, 'utf8');
                $title_inp = $_GET['title'];
                $sql2 = "SELECT title_art, description_art, text_art, code_art, date_create_art FROM articulos WHERE title_art = ?";
                $sql_result_prepare = mysqli_prepare($connection2, $sql2);
                $prepare_parameter = mysqli_stmt_bind_param($sql_result_prepare, 's', $title_inp);
                $consult_execute = mysqli_stmt_execute($sql_result_prepare);
                if ($consult_execute){
                    $consult_result = mysqli_stmt_bind_result($sql_result_prepare, $title,
                        $description, $text, $code, $datatieme);
        ...
        <h2 class="bg-info py-2 text-center text-dark mb-3">Articulos</h2>
        <div class="row">
            <?php
